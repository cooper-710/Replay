...
let spinAxis = new THREE.Vector3(0, 1, 0); // fallback spin axis (vertical)
let spinSpeed = 0;
ball = null;
...
ball = null, balls = [], trailDots = [], showTrail = false, playing = true;
let pitchData = [];
...
function throwPitch(pitch) {
  ...
  ball.position.set(
    ...
    ball.userData.release.z
      ...
      if (ball && typeof ball.rotateOnAxis === 'function' && spinAxis && spinAxis.x !== undefined) {
        ball.rotateOnAxis(spinAxis, spinSpeed);
      }
      if (ball && spinAxis instanceof THREE.Vector3 && spinSpeed > 0) ball.rotateOnAxis(spinAxis, spinSpeed);

    
      ball.rotateOnAxis(spinAxis, spinSpeed);playing = true;
  document.getElementById('toggleBtn').textContent = 'Pause';
  scene.add(ball);
...
document.getElementById('replayBtn').addEventListener('click', () => {
  ...
  ball.position.set(
    ...
    ball.userData.release.z
      ...
      ball.rotateOnAxis(spinAxis, spinSpeed);for (const d of trailDots) scene.remove(d.mesh);
  trailDots = [];
});
