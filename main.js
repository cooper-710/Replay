...
function populateSelectors() {
  ...
  batterSel.addEventListener('change', updateDropdowns);
  dateSel.addEventListener('change', updatePitchersAndPitches);
  pitcherSel.addEventListener('change', updatePitchDropdown);
  ...
  function updateDropdowns() {
    ...
    const filteredByBatter = pitchData.filter(p => p.batter === batter);
    const dates = [...new Set(filteredByBatter.map(p => p.game_date))].sort();
    dateSel.innerHTML = '';
    dates.forEach(d => dateSel.add(new Option(d, d)));
    updatePitchersAndPitches();
  }

  function updatePitchersAndPitches() {
    const batter = batterSel.value;
