...
// === SPIN UPDATE ===
if (ball.userData.spinRate > 0 && ball.userData.spinAxis) {
  const radPerSec = (ball.userData.spinRate / 60) * 2 * Math.PI;
  const angleDelta = radPerSec * delta;
  ball.rotateOnAxis(ball.userData.spinAxis.clone().normalize(), angleDelta);
}
