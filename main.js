...
function throwPitch(pitch) {
  ...
  ball.castShadow = true;
    ...
    // === DEBUG SPIN BAR ===
    const barGeometry = new THREE.CylinderGeometry(0.01, 0.01, 0.4, 8);
    const barMaterial = new THREE.MeshBasicMaterial({ color: 0xff00ff });
    const spinBar = new THREE.Mesh(barGeometry, barMaterial);
    spinBar.rotation.z = Math.PI / 2;
    spinBar.position.set(0, 0, 0.16); // offset from center
    ball.add(spinBar);
    ball.userData.spinBar = spinBar;
