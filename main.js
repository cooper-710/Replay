...
let balls = [], trailDots = [], showTrail = false, playing = true, pitchData = [];
let lastTime = 0;
...
function addBall(pitch, pitchTypeLabel) {
  console.log("ADDING BALL:", pitchTypeLabel);
  console.log("POSITION:", -pitch.release_pos_x, pitch.release_pos_z, -pitch.release_pos_y);
  console.log("SPIN RATE:", pitch.release_spin_rate, "AXIS:", pitch.spin_axis);
...
function animate() {
  ...
  if (playing) {
    for (let ball of balls) {
      ...
      scorebug.innerHTML = `
        <b>${ball.userData.pitcher} vs ${ball.userData.batter}</b><br>
        <span>${ball.userData.inning} ${ball.userData.top_bottom} | ${ball.userData.balls}-${ball.userData.strikes}, ${ball.userData.outs} outs</span><br>
        <span><b>${ball.userData.pitch_type}</b> – ${ball.userData.description}</span>
      `;
...
function populateSelectors() {
  ...
  pitcherSel.addEventListener('change', () => {
    ...
    balls = [];
    trailDots.forEach(dot => scene.remove(dot.mesh));
    trailDots = [];
    const batter = batterSel.value;
    const date = dateSel.value;
    const pitcher = pitcherSel.value;
    const filtered = pitchData.filter(p => p.batter === batter && p.game_date === date && p.pitcher === pitcher);
  ...
  pitchSel.addEventListener('change', () => {
    ...
    addBall(pitch, pitch.pitch_type);
scorebug.innerText = `⚾ ${pitch.pitcher} vs ${pitch.batter} | Inning ${pitch.inning} ${pitch.top_bottom} | Count: ${pitch.balls}-${pitch.strikes} | ${pitch.pitch_type} - ${pitch.description}`;
  });
}
...
document.getElementById('trailToggle').addEventListener('change', e => {
  ...
  if (!showTrail) {
    trailDots.forEach(d => scene.remove(d.mesh));
    trailDots = [];
  }
});

document.getElementById('replayBtn').addEventListener('click', () => {
  balls.forEach(ball => { ball.userData.t0 = clock.getElapsedTime(); });
  playing = true;
...
scorebug.style.position = "absolute";
scorebug.style.top = "10px";
scorebug.style.left = "10px";
scorebug.style.padding = "10px 14px";
scorebug.style.background = "rgba(0, 0, 0, 0.85)";
scorebug.style.color = "#ffffff";
scorebug.style.fontFamily = "monospace";
scorebug.style.fontSize = "14px";
scorebug.style.borderRadius = "8px";
scorebug.style.boxShadow = "0 2px 6px rgba(0,0,0,0.3)";
scorebug.style.zIndex = "999";
scorebug.innerText = "⚾ Awaiting pitch selection...";
document.body.appendChild(scorebug);
