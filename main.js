...
function setupScene() {
  ...
  const mound = new THREE.Mesh(moundGeometry, new THREE.MeshStandardMaterial({ color: 0x8B4513 }));
...
function populateSelectors() {
  ...
  const batters = [...new Set(pitchData.map(p => p.batter))].sort();
  batters.forEach(b => batterSel.add(new Option(b, b)));

  batterSel.addEventListener('change', updateFiltered);
  dateSel.addEventListener('change', updateFiltered);
  pitcherSel.addEventListener('change', updateFiltered);
  ...
  function updateFiltered() {
    ...
    const dates = [...new Set(pitchData.filter(p => p.batter === selectedBatter).map(p => p.game_date))];
    dateSel.innerHTML = '';
    dates.forEach(d => dateSel.add(new Option(d, d)));

    const selectedDate = dateSel.value;
    const filtered = pitchData.filter(p => p.batter === selectedBatter && p.game_date === selectedDate);
    const pitchers = [...new Set(filtered.map(p => p.pitcher))];
    pitcherSel.innerHTML = '';
    pitchers.forEach(p => pitcherSel.add(new Option(p, p)));

    const selectedPitcher = pitcherSel.value;
    const selected = pitchData.filter(p => p.batter === selectedBatter && p.game_date === selectedDate && p.pitcher === selectedPitcher);
    pitchSel.innerHTML = '';
  ...
  batterSel.dispatchEvent(new Event('change'));
}
