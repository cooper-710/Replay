...
let ballPivot = null, ball = null, trailDots = [], showTrail = false, playing = true;
let pitchData = [];
...
function throwPitch(pitch) {
  if (ballPivot) scene.remove(ballPivot);
...
function animate() {
  ...
  if (playing && ball) {
    ...
    if (showTrail) {
      ...
      dot.position.copy(ballPivot.position);
      scene.add(dot);
      trailDots.push({ mesh: dot, t0: now });
    }

    if (ballPivot.position.z <= -60.5 && playing) {
      playing = false;
